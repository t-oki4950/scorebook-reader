...
現在の基本運用は、1打席分の画像を貼り付け、Codexが投球記号と順序、カウント、打撃結果、走塁状況を仮読み取りし、人間が添削して事例・ルールへ蓄積する流れです。
...
人間が添削するときは、打席フォームの「投球記号と順序」「カウント」「打撃結果」「走者イベント」「備考」を直します。修正後に保存し、読み取り事例として追加すると、次回以降のCodex読解に再利用できます。
